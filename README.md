...
SysMLD is a lightweight diagram/view format and deterministic rendering toolkit for [SysML v2](https://www.omg.org/spec/SysML) models: **`.sysml` + intent JSON + `.sysmld` + SVG**. This repo is an open-source replacement path for expensive tool-locked SysML 2 suites.

## SysML2d and MSML

These are **paired toolchains**, not two names for one language.

| | **This repo (SysML2d)** | **[MSML](https://github.com/meaningfulsystems/msml)** |
| --- | --- | --- |
| Language | SysML v2 | SysML 1-inspired (not SysML v2) |
| Files | `.sysml` · intent JSON · `.sysmld` · SVG | `.msml` · `.msmd` · PNG |
| Shared | E-bike *names* (frozen) | Same e-bike names — different language |
...
The same idea — models, diagrams, and history as text — also drives the paired [MSML](https://github.com/meaningfulsystems/msml) toolchain. The two stay separate: SysML2d is SysML v2 (this repo); MSML is a SysML 1-inspired language, not a second SysML v2 suite.
...
AI agents: [AGENTS.md](AGENTS.md) and the four skills (`bootstrap-project`, `author-model`, `compose-views`, `vision-review`). Connections must never pass over boxes.
