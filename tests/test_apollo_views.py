...
class ApolloViewTests(unittest.TestCase):
    def test_apollo_intents_compose_validate_and_render(self):
        ...
        self.assertIn("under\nrate of descent.", land)
        self.assertNotIn("P66", land)
        self.assertNotIn("program P66", land)
        self.assertNotRegex(land, r"(?i)\bfps\b")
        self.assertNotRegex(land, r"(?i)ft/s")
        self.assertNotRegex(joined, r"(?i)\bfps\b")
        self.assertNotRegex(joined, r"(?i)ft/s")
        ...
        self.assertIn("The Range Safety Officer\nshall command\nultra-high-frequency (UHF)\ndestruct from outside", req["nodes"]["safety"]["label"])
    ...
    def test_apollo_locked_msml_names(self):
        ...
        self.assertEqual(stm_spec["states"]["dockEject"]["label"], "dock and eject")
        ...
        self.assertIn("The Range Safety Officer shall command ultra-high-frequency (UHF) destruct from outside Mission Control until that command is safed after Earth orbit.", text)
        self.assertIn("The Lunar Module shall land two crew on the Moon under rate of descent.", text)
        ...
        self.assertNotIn("dockEject", note)
        self.assertNotIn("surfaceEVA", note)
        self.assertNotIn("Hyphens are not legal", note)
        self.assertNotIn("SIC →", note)
        self.assertNotIn("dock/eject", note)
        self.assertIn("dock and eject", note)
        self.assertIn("rate of descent (P66)", note)
        self.assertNotRegex(note, r"(?i)\bfps\b")
        self.assertNotRegex(note, r"(?i)ft/s")
        self.assertIn("Translunar Injection → dock and eject → translunar coast → Lunar Orbit Insertion", note)
        ...
        self.assertNotIn("P30", lunar_labels)
        right_offsets = {
